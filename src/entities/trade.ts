...
import { ONE, ZERO, _10000 } from '../constants'
...
export class Trade<TInput extends Currency, TOutput extends Currency, TTradeType extends TradeType> {
   ...
   */
  public readonly optimalAmountOut?: CurrencyAmount<Token>
  ...
  public constructor(
    ...
    if (tradeType === TradeType.EXACT_INPUT) {
      ...
      for (let i = 0; i < route.path.length - 1; i++) {
        ...
        const [outputAmount, optimalAmountOut, nextPair] = pair.getOutputAmount(tokenAmounts[i])
        tokenAmounts[i + 1] = outputAmount
        nextPairs[i] = nextPair
        this.optimalAmountOut = optimalAmountOut
      }
    ...
    this.executionPrice = new Price(
      ...
      this.inputAmount.numerator,
      this.outputAmount.numerator
    )
    this.nextMidPrice = new Route(nextPairs, route.input, route.output).midPrice
  ...
  public minimumAmountOut(slippageTolerance: Percent): CurrencyAmount<TOutput> {
    ...
    if (this.tradeType === TradeType.EXACT_OUTPUT) {
      ...
      const slippageAdjustedAmountOut = new Fraction(ONE)
        ...
        .invert()
        .multiply(this.outputAmount.numerator).quotient
  ...
  public maximumAmountIn(slippageTolerance: Percent): CurrencyAmount<TInput> {
    ...
    if (this.tradeType === TradeType.EXACT_INPUT) {
      ...
      const slippageAdjustedAmountIn = new Fraction(ONE).add(slippageTolerance).multiply(this.inputAmount.numerator)
        .quotient
  ...
  public static bestTradeExactIn<TInput extends Currency, TOutput extends Currency>(
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      let amountOut: CurrencyAmount<Token>, optimalAmountOut: CurrencyAmount<Token>
      try {
      ...
      const rawOptimalAmountOut = optimalAmountOut.numerator
      ...
      if (JSBI.lessThanOrEqual(amountOutWithSlippage, amountOut.numerator)) {
