import { CurrencyAmount, Ether, Percent, SupportedChainId, Token, TradeType, WETH9 } from '@uniswap/sdk-core'
import JSBI from 'jsbi'
import { Pair, Route, Trade, TradeState } from '../src'
...
describe('Trade', () => {
  ...
  const token2 = new Token(SupportedChainId.MAINNET, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(SupportedChainId.MAINNET, '0x0000000000000000000000000000000000000004', 18, 't3')
  const ETHER = Ether.onChain(SupportedChainId.MAINNET)

  const pair_0_1 = BasicPair(
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(1000))
  )
  const pair_0_2 = BasicPair(
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(1100))
  )
  const pair_0_3 = BasicPair(
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token3, JSBI.BigInt(900))
  )
  const pair_1_2 = BasicPair(
    CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(1200)),
    CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(1000))
  )
  const pair_1_3 = BasicPair(
    CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(1200)),
    CurrencyAmount.fromRawAmount(token3, JSBI.BigInt(1300))
  )

  const pair_weth_0 = BasicPair(
    CurrencyAmount.fromRawAmount(WETH9[SupportedChainId.MAINNET], JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000))
  )

  const empty_pair_0_1 = BasicPair(
  ...
  describe('#bestTradeExactIn', () => {
    it('throws with empty pairs', () => {
      expect(() =>
        Trade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token2, 10)
      ).toThrow('PAIRS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        Trade.bestTradeExactIn([pair_0_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token2, 10, {
          maxHops: 0
        })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(
        Trade.bestTradeExactIn([empty_pair_0_1], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token1, 10)
      ).toHaveLength(0)
    })
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result[0].route.path).toEqual([WETH9[SupportedChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].route.path).toEqual([WETH9[SupportedChainId.MAINNET], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH9[SupportedChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH9[SupportedChainId.MAINNET]])
